  ...
  return (
    <div className="panel">
          ...
          return (
            <div
              ...
              className={['seat', empty ? 'open' : '', seat.playerId === myPlayerId ? 'mine' : '']
                .join(' ')
                .trim()}
              onClick={empty ? () => pickSeat(seat.seat) : undefined}
            >
      ...
      <div className="actions">
        <button onClick={toggleReady}>{mySeat?.ready ? 'Not ready' : 'Ready'}</button>
        {isHost && (
          <button className="secondary" onClick={start} disabled={!allFull || !allReady}>
            Start game
          </button>
        )}
      </div>
